...
static int cmp_sif(const void* a, const void* b)
{
    const Process* p1 = (const Process*)a;
    const Process* p2 = (const Process*)b;
    int A = 999, B = 999;
    if (p1->io_index < p1->io_num)
        A = p1->io_start_time[p1->io_index] - p1->burst_time + p1->remaining_time;
    if (((Process*)b)->io_index < ((Process*)b)->io_num)
        B = p2->io_start_time[p2->io_index] - p2->burst_time + p2->remaining_time;
    if (A != B)
        return A - B;
    return p1->arrival_time - p2->arrival_time;
}

void run_sif_np(Process* processes, int n)
{
    int current_time = 0;
    ...
    {
        qsort(processes, n, sizeof(Process), cmp_sif);
        int pt = -1;
        while (pt == -1)
        {
            for (int i = 0; i < n; ++i)
